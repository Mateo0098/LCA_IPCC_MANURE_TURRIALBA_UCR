...
def tabla_04_parametros_modelo_acv() -> Path:
    ...
    for _, row in df.iterrows():
        for col, name, unit, source_col, obs in param_defs:
            row_observation = obs
            if col == "n_ex_fraction" and row["escenario"] == "A" and int(row["etapa"]) == 2:
                row_observation = (
                    "Benchmark experimental de A2 en base húmeda; no reinicializa el ledger "
                    "productivo, que recibe N total y TAN propagados desde A1"
                )
            rows.append({
...
def tabla_09_comparacion_escenarios() -> Path:
    ...
    for category, col, unit in defs:
        ...
        rows.append({
            ...
            "fuente": "processed/acv_impacto_total_por_escenario.csv",
            "observaciones": "Comparación de los resultados del ledger productivo de N total y TAN propagado entre etapas",
        })
...
def resumen_resultados_para_redaccion() -> Path:
    ...
    lines.extend(
        [
            ...
            "A2, correspondiente a la Etapa 2: Lombricompostaje, se estimó mediante la arquitectura híbrida IPCC–EMEP–Komakech vigente.",
            "",
