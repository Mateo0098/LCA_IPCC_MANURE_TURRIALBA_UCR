...
def build_document() -> None:
    ...
    add_paragraphs(doc, [
        ...
        "El estiércol fresco constituyó el flujo de referencia del sistema y fue caracterizado mediante humedad, materia seca, cenizas, sólidos volátiles y nitrógeno total. El estiércol precompostado caracterizó el material obtenido tras A1 y disponible al ingreso de A2; no representa el producto final del lombricompostaje.",
    ...
    add_paragraphs(doc, [
        "El nitrógeno total reportado en porcentaje se convirtió a fracción másica según la función metodológica de cada frontera. Para el estiércol fresco analizado por LASA mediante Kjeldahl, así como para aguas verdes y purines, se conservó el tratamiento directo sobre la muestra o alícuota analizada, sin multiplicar por materia seca.",
        "En el precompostado, el CIA determinó N por Dumas sobre muestra seca o acondicionada a 80 °C durante 48 h. Este procedimiento fue distinto de la gravimetría de Bioenergía a 105 °C durante 16 h. El porcentaje integrado de N se convirtió a base húmeda con la materia seca gravimétrica exclusivamente para construir el benchmark experimental de A2. El acondicionamiento CIA no se utilizó para reconstruir humedad ni se corrigieron posibles pérdidas de N durante esa preparación.",
        "El benchmark experimental del precompostado no reinicializó el balance productivo de A2. El N total y el nitrógeno amoniacal total de entrada a A2 fueron los remanentes propagados desde A1. El carbono y la relación C/N se conservaron únicamente como caracterización descriptiva, sin conversión a base húmeda ni participación en el ACV.",
    ...
    add_latex_equation(doc, r"R_{j}=\frac{f_{\mathrm{MS,fresco},j}\,f_{\mathrm{cenizas,fresco},j}}{f_{\mathrm{MS,precompostado},j}\,f_{\mathrm{cenizas,precompostado},j}};\quad \widehat m_{\mathrm{húmeda,A2}}=m_{\mathrm{húmeda,A1}}\,\overline R", ["Donde: R_j = razón estimada de masas húmedas en la jornada j, kg/kg; las fracciones de materia seca se refieren a masa húmeda y las de cenizas a materia seca; la barra representa la media de los factores de las jornadas elegibles con igual peso temporal. El sombrero identifica una masa inferida, no pesada a la salida de A1."])
    add_paragraphs(doc, [
        "La conservación de cenizas es un supuesto de trazador: presupone que el material comparado representa la misma corriente y que no entra ni sale ceniza mineral. Cada razón se calculó con los promedios de la jornada antes de integrar las razones. La masa anual estimada a la entrada de A2 no es una medición de salida de A1; tampoco se exige que la razón integrada conserve exactamente las cenizas al combinarla con concentraciones integradas por separado.",
        "La disminución estimada de masa húmeda no identifica por sí misma evaporación, degradación de sólidos, emisiones gaseosas ni drenaje. Se midió la humedad de muestras de estiércol fresco y precompostado, pero no los caudales de agua añadida, evaporada o drenada ni la masa de los sólidos perdidos entre ambas fronteras. Por ello no se cerró un balance cuantitativo de agua. El agua de lavado cuantificada corresponde a la ruta de aguas verdes y no se asignó a A1 ni a A2; la ausencia de riego regular observada en A2 tampoco equivale a demostrar un flujo de agua nulo.",
        "La masa húmeda inferida sirve como base de actividad del metano de manejo y del factor de amoníaco de A2 expresado por masa de residuo entrante. El N total y el nitrógeno amoniacal total de A2 proceden del balance secuencial de A1, con pérdidas de N definidas por sus mecanismos específicos. La concentración analítica del precompostado multiplicada por la masa inferida constituye solo un contraste experimental independiente.",
    ])
    ...
    add_paragraphs(doc, [
        ...
        "La capacidad máxima de producción de metano se fijó en B₀ = 0,24 m³ CH₄/kg SV, default del IPCC 2019 para ganado lechero de sistemas de alta productividad en otras regiones y no medición local. La evidencia histórica publicada del mismo Módulo Lechero por Conejo-Morales y WingChing-Jones (2020) respalda esa clasificación frente a la alternativa de baja productividad, sin atribuir al hato de 2026 los rendimientos históricos ni interpretar el artículo como una medición de B₀. El factor 0,67 kg CH₄/m³ procede de la ecuación 10.23 del IPCC. AWMS = 1 se aplica a la corriente ya asignada a cada etapa y no al estiércol total de la finca.",
        ...
        "A2: Lombricompostaje utilizó una arquitectura híbrida explícita y trazable. La categoría IPCC Composting – Passive Windrow se adoptó como proxy metodológico para estimar CH₄ y N₂O directo. La fracción de lixiviación se estableció en cero como parámetro específico de las condiciones operativas modeladas, sin afirmar imposibilidad física universal. La fracción IPCC de volatilización se conservó únicamente como benchmark de orden de magnitud y no generó una emisión adicional. Esta arquitectura constituye una decisión metodológica vigente y no la aplicación íntegra de una sola guía.",
        "La especiación del N de A2 combinó fuentes metodológicas. El NH₃ se estimó con el factor de 12,8 g NH₃ por Mg de residuo orgánico de entrada de Komakech et al. (2016), aplicado una sola vez a la masa húmeda inferida que ingresa a A2. El artículo obtuvo ese factor para reactores pequeños en Kampala, con residuo predominantemente ganadero, a partir de concentraciones gaseosas medidas y un flujo de aire estimado mediante el índice respirométrico dinámico; su transferencia al sistema de Turrialba se clasificó como proxy experimental aprobado y extrapolación con limitaciones. El denominador se interpretó como masa húmeda de entrada porque el estudio formuló su unidad funcional y balance por tonelada de residuo o estiércol húmedo, aunque la expresión puntual del factor utiliza solamente el término residuo biológico. NO-N y N₂-N conservaron factores EMEP/EEA de almacenamiento sólido como proxies metodológicos aprobados provisionalmente ante la ausencia de una categoría específica de lombricompostaje. El factor de N₂-N de 0,300 kg/kg TAN se mantuvo como fuente importante de incertidumbre de representatividad, sin tratarlo como error ni sustituirlo por una relación IPCC N₂:N₂O.",
    ...
    add_paragraphs(doc, [
        ...
        "En A1 se aplicaron los factores EMEP de almacenamiento sólido sobre TAN. El drenaje de A1 redujo el N total y llegó primero al suelo; solamente la fracción posteriormente lixiviada o escurrida desde el suelo originó NO₃⁻. En A2, el NH₃ se calculó con el proxy experimental de 12,8 g NH₃ por Mg de residuo orgánico húmedo de entrada (Komakech et al., 2016), mientras NO y N₂ utilizaron los factores EMEP sólidos como aproximación ante la ausencia de una categoría explícita de lombricompostaje.",
        "El uso consecutivo de factores en A1 y A2 no duplicó el mismo N: A2 recibió el N total y el pool residual de TAN a la salida de A1. Las duraciones específicas de ambas etapas introducen incertidumbre de representatividad temporal y de transferencia de los proxies, pero no justifican un escalado lineal ni demuestran doble conteo.",
    ...
    add_paragraphs(doc, [
        "Conejo-Morales, J. F., & WingChing-Jones, R. (2020). Condiciones climáticas y la producción láctea del ganado Jersey en dos pisos altitudinales. Agronomía Mesoamericana, 31(1), 157–175. https://doi.org/10.15517/am.v31i1.34739",
