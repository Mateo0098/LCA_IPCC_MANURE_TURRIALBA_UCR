...
def build_rows() -> list[dict[str, str | float]]:
    ...
    factors = {(row["flujo_elemental"], row["categoria_impacto"]): row for row in read_rows(FACTORS)}
    ...
    cf_ch4 = number(factors[("Methane biogenic", "Cambio climático")], "factor")
    cf_n2o = number(factors[("Nitrous oxide", "Cambio climático")], "factor")
