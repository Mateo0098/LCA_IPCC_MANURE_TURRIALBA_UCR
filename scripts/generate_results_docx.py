...
def build_document() -> None:
    ...
    add_paragraphs(
        ...
        [
            ...
            f"A1: Precomposteo recibió {fmt(flow_values['A1: Precomposteo'], 2)} kg/año de estiércol fresco recolectado. La entrada húmeda de A2: Lombricompostaje se estimó en {fmt(flow_values['A2: Lombricompostaje'], 2)} kg/año mediante una razón de masa derivada de materia seca y cenizas por jornada, bajo el supuesto de conservación de estas últimas. Esa entrada no fue pesada como salida de A1. La diferencia entre ambas masas húmedas no permite cuantificar evaporación ni pérdidas de sólidos: se dispuso de humedades de muestras, pero no de los flujos de agua y sólidos necesarios para cerrar ambos balances.",
            "El N total y el nitrógeno amoniacal total de A2 se propagaron desde A1 según las pérdidas de N modeladas. La masa húmeda estimada no se utilizó para descontar N de forma adicional; el N analítico del precompostado permanece como contraste experimental independiente.",
    ...
    add_paragraphs(
        ...
        [
            ...
            "A2: Lombricompostaje se calculó mediante una arquitectura híbrida explícita y trazable. IPCC y la categoría Composting – Passive Windrow cubrieron CH₄ y N₂O directo; la fracción de lixiviación se estableció en cero como parámetro específico de las condiciones operativas modeladas, sin alterar el valor genérico de la categoría. La Tabla 3 resume los parámetros principales.",
            "La fracción IPCC de volatilización se conservó solo como benchmark y no generó una segunda emisión. El NH₃ se estimó una sola vez con el proxy experimental aprobado de Komakech et al. (2016), aplicado a la masa húmeda inferida de entrada; NO y N₂ emplearon proxies EMEP/EEA de almacenamiento sólido aprobados provisionalmente. Estas elecciones son decisiones metodológicas vigentes, aunque conservan incertidumbres de transferibilidad y representatividad, especialmente para el factor de N₂.",
    ...
    add_paragraphs(
        ...
        [
            ...
            f"{dominant_emission('CH4')} presentó la mayor contribución de CH4. {dominant_emission('N2O')} presentó la mayor emisión de N2O. A2: Lombricompostaje fue estimada mediante la arquitectura híbrida IPCC–EMEP–Komakech vigente. La Tabla 4 resume las emisiones anuales por escenario y sustancia, y la Figura 4 presenta las emisiones de CH4 por etapa.",
    ...
    add_paragraphs(doc, [
        "Los resultados oficiales de A2: Lombricompostaje se contrastaron con la referencia empírica de Jjagwe et al. (2019) como benchmark interpretativo, sin reinicializar el ledger, forzar NH₃ nulo, sustituir la arquitectura IPCC–EMEP–Komakech ni crear un segundo inventario oficial. La base común fue la materia seca del estiércol precompostado al ingreso de A2.",
